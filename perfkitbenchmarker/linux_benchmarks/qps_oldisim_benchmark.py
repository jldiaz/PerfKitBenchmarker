...
This benchmark is based on oldisim (see that one for details). The difference
is that qps_oldisim reports also the QPS reached and not only the scaling
efficience.

Sample command line:

./pkb.py --benchmarks=qps_oldisim --project='YOUR_PROJECT' --qps_oldisim_num_leaves=4
--qps_oldisim_fanout=1,2,3,4 --qps_oldisim_latency_target=40
--qps_oldisim_latency_metric=avg
...
flags.DEFINE_integer('qps_oldisim_num_leaves', 4, 'number of leaf nodes',
                     lower_bound=1, upper_bound=64)
flags.DEFINE_list('qps_oldisim_fanout', [],
...
flags.DEFINE_enum('qps_oldisim_latency_metric', 'avg',
                  ['avg', '50p', '90p', '95p', '99p', '99.9p'],
                  'Allowable metrics for end-to-end latency')
flags.DEFINE_float('qps_oldisim_latency_target', '30', 'latency target in ms')
...
BENCHMARK_NAME = 'qps_oldisim'
BENCHMARK_CONFIG = """
qps_oldisim:
  description: >
      Run qps_oldisim. Specify the number of leaf
      nodes with --qps_oldisim_num_leaves
  vm_groups:
    default:
...
def GetConfig(user_config):
  """Decide number of vms needed to run qps_oldisim."""
  config = configs.LoadConfig(BENCHMARK_CONFIG, user_config, BENCHMARK_NAME)
  config['vm_groups']['default']['vm_count'] = (FLAGS.qps_oldisim_num_leaves
...
def RunLoadTest(benchmark_spec, fanout):
  ...
  Returns:
    A tuple of (peak_qps, peak_lat, target_qps, target_lat).
  """
  assert fanout <= FLAGS.qps_oldisim_num_leaves, (
      ...
      'leaves %s.' % (fanout, FLAGS.qps_oldisim_num_leaves))
  ...
  driver_cmd = '%s -s %s:%s -t 30 -- %s %s --threads=%s --depth=16' % (
      launch_script, FLAGS.qps_oldisim_latency_metric, FLAGS.qps_oldisim_latency_target,
...
def Run(benchmark_spec):
  ...
  fanout_list = set([1, FLAGS.qps_oldisim_num_leaves])
  for fanout in map(int, FLAGS.qps_oldisim_fanout):
    if fanout > 1 and fanout < FLAGS.qps_oldisim_num_leaves:
      fanout_list.add(fanout)

  metadata = {'num_cpus': vm.num_cpus, 'cpu_info': vm.cpu_info}
  ...
  for fanout in sorted(fanout_list):
    ...
    results.append(sample.Sample("QPS for %s leaves"%fanout, qps_dict[fanout], '', metadata))
